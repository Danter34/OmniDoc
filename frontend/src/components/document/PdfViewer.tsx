...
export function PdfViewer({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-slate-100">
      <div className="flex min-h-16 shrink-0 flex-wrap items-center gap-2 border-b border-slate-200 bg-white px-3 py-2">
        <div className="mr-auto min-w-0 basis-[12rem]">
          <label className="sr-only" htmlFor="pdf-viewer-document-selector">
            Tài liệu đang xem
          </label>
          <select
            className="block h-7 w-full truncate rounded-md border-0 bg-transparent pr-1 text-sm font-semibold text-slate-900 outline-none hover:bg-slate-50 focus:ring-2 focus:ring-blue-500/20"
        ...
        <div className="flex items-center rounded-lg border border-slate-200 bg-slate-50 p-0.5">
          <button
            ...
            className="flex size-8 items-center justify-center rounded-md text-slate-600 transition hover:bg-white disabled:opacity-40"
          ...
          <label className="flex items-center gap-1 px-1 text-xs text-slate-500">
            ...
            <input
              ...
              className="h-7 w-11 rounded border border-slate-200 bg-white px-1 text-center font-medium tabular-nums text-slate-800 outline-none focus:border-blue-500"
        ...
        <div className="flex items-center rounded-lg border border-slate-200 bg-slate-50 p-0.5">
          ...
          <span className="w-11 text-center text-xs font-medium tabular-nums text-slate-600">
          ...
          <button
            ...
            className={cn(
              "flex size-8 items-center justify-center rounded-md text-slate-600 transition hover:bg-white disabled:opacity-40",
              zoom === "page-width" && "bg-white text-blue-600 shadow-sm",
            )}
            disabled={isLoading || Boolean(error)}
      ...
      <div className="relative min-h-0 flex-1 overflow-hidden bg-slate-200/70">
        {isLoading ? <PdfViewerSkeleton /> : null}

        {!isLoading && error ? (
          <div className="flex h-full items-center justify-center p-6 text-center">
            <div className="max-w-sm rounded-2xl border border-rose-200 bg-white p-6 shadow-sm">
              <span className="mx-auto flex size-11 items-center justify-center rounded-xl bg-rose-50 text-rose-600">
        ...
        {!isLoading && !error && viewerUrl ? (
          <>
            ...
            <iframe
              ...
              src={viewerUrl}
              title={`Tài liệu PDF ${document.fileName}, trang ${pageNumber}`}
            />
          </>
        ) : null}
        ...
        {target?.fromCitation && !error ? (
          <div
            aria-live="polite"
            className="pdf-citation-notice pointer-events-none absolute bottom-4 left-1/2 z-20 -translate-x-1/2 whitespace-nowrap rounded-full bg-amber-500 px-3.5 py-2 text-xs font-semibold text-white shadow-lg shadow-amber-900/20"
...
function PdfViewerSkeleton() {
  return (
    <div className="flex h-full flex-col items-center gap-4 overflow-hidden px-6 py-8">
      ...
      <div className="h-full w-full max-w-2xl animate-pulse rounded-md bg-white shadow-sm">
