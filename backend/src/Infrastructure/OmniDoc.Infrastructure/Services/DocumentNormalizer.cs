...
public sealed class DocumentNormalizer(PassThroughPdfNormalizer pdf, GotenbergChromiumNormalizer chromium, GotenbergLibreOfficeNormalizer office, CsvCanonicalPdfNormalizer csv) : IDocumentNormalizer
{
    public Task<CanonicalPdfResult> NormalizeAsync(Stream sourceStream, DocumentFormat format, CancellationToken ct) =>
        format switch
        {
            ...
            DocumentFormat.Docx or DocumentFormat.Pptx or DocumentFormat.Xlsx => office.NormalizeAsync(sourceStream, format, ct),
            DocumentFormat.Csv => csv.NormalizeAsync(sourceStream, format, ct),
